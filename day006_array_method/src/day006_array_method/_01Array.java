...
public class _01Array {
	...
	public static void main(String[] args) {
		 ...
		 *  需求:想要保存全班80个同学的考试成绩
		 *  声明80个int类型的变量，保存。
		 *  存在问题：1. 声明太多变量，起名困难。 2. 容易命名冲突 
		 *  3. 代码冗余
		 *  怎么解决上面问题？
		 *  通过数组
		 */
		// 1. 声明数组
		// 1.1 动态创建, 语法：数据类型[] 数组名 = new 数据类型[长度];//长度不能为负数，且是int类型 最大就是int最大值2147483647
		int[] arr = new int[3];// 声明一个int类型数组，命名arr，长度是3。
		// 2. 数组赋值,语法：数组名[下标] = 值;
		arr[0] = 1;// 给数组arr第1个元素赋值1
		arr[1] = 2;// 给数组arr第2个元素赋值2
		arr[2] = 3;// 给数组arr第3个元素赋值3
		// 3. 数组取值，语法：数组名[下标] （取出来：1. 打印输出 2.比较大小 3.计算 4.赋值给其他变量 5.作为方法的参数使用）
